...
<html>
    <head>
        ...
        <link rel="stylesheet" id="main-stylesheet"
              href="{{ asset('assets/css/custom-stylesheets', $settings->get('enable_https')).'/'.(($settings->get('selected_sheet') && ! IS_DEMO) ? $settings->get('selected_sheet').'/styles.min.css?v25' : 'original/styles.min.css?v25') }}">
    ...
    <div id="splash" ng-cloak>
        ...
        <script id="vars">
            var vars = {
                ...
                isDemo: '{{ $isDemo  }}',
                message: "{{ Session::get('jsMessage') }}",
                version: "{{ VERSION }}",
            }
        </script>

        <script src="{{ asset('assets/js/core.min.js?v25', $settings->get('enable_https')) }}"></script>
