...
.factory('player', function($rootScope, $http, $timeout, $injector, localStorage, utils, html5Player) {
    var player = {
        ...
        loadTrack: function(track, autoPlay) {
            ...
            setCorrectPlayerBackend(track);

            if ((! track.youtube_id && ! track.url) || utils.isDemo) {
                fetchTrack(track).success(function(data) {
                    ...
                    track.youtube_id = data.id;

                    if (autoPlay) {
                        player.playerBackend.loadVideo(track, autoPlay, 'large');
                        player.play();
                    } else {
                        player.playerBackend.cueVideo(track, autoPlay, 'large');
                    }
                });
            } else {
                if (autoPlay) {
        ...
        goFullScreen: function() {
        	if (this.playerBackend.html5Player) {
				var thisVid = document.getElementById('html5-player');
        	} else {
        ...
        init: function() {
            ...
            this.playerService = utils.getSetting('audio_search_provider', 'youtube').toLowerCase()+'Player';

            this.playerBackend = $injector.get(this.playerService);
    ...
    /**
     * Switch between youtube/soundcloud and html5 player depending
     * on available sources for the requested track
     *
     * @param {object} track
     */
    function setCorrectPlayerBackend(track) {
        if (track.url) {
            player.stop();
            if ( ! html5Player.bootstraped) html5Player.init(player);
            player.playerBackend = html5Player;
            document.getElementById('player').classList.add('hidden');
            document.getElementById('html5-player').classList.remove('hidden');
        } else if ( ! player.playerBackend[player.playerService]) {
            player.stop();
            player.playerBackend = $injector.get(player.playerService);
