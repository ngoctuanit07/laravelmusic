...
class SitemapGenerator {
    ...
    private function save($fileName)
    {
        ...
        Storage::put('sitemaps/'.$fileName.'.xml', $this->xml);
     ...
     * @param object $item
     */
    private function addNewLine($name = false, $item = false, $url = false, $updatedAt = false)
    {
    ...
    {
        $this->getModel($name, $columns)->chunk($this->queryLimit, function($items) use($name)
        {
            if ($name === 'albums') {
    ...
    private function makeStaticMap()
    {
        ...
        $this->addNewLine(false, false, $this->baseUrl.'top-albums', $this->getItemUpdatedAtTime());
    ...
    private function makeIndex(array $index)
    {
        ...
        foreach ($index as $resource => $number) {
            for ($i=1; $i <= $number; $i++) {
                $url = url("/sitemaps/{$resource}-sitemap-$i.xml");
                $string .= "\t<sitemap>\n"."\t\t<loc>$url</loc>\n"."\t\t<lastmod>{$this->getItemUpdatedAtTime()}</lastmod>\n"."\t</sitemap>\n";
            }
        }

        $string .= "\t<sitemap>\n"."\t\t<loc>".url("/sitemaps/static-urls-sitemap.xml")."</loc>\n"."\t\t<lastmod>{$this->getItemUpdatedAtTime()}</lastmod>\n"."\t</sitemap>\n";

        $string .= '</sitemapindex>';

        Storage::put('sitemaps/sitemap-index.xml', $string);
    }
}
